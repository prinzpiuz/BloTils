...
func TestGetClapsWithBody(t *testing.T) {
	...
	mock.ExpectQuery(
		`^SELECT\s+Likes\.id,\s+Likes\.uri,\s+Likes\.domain_id,\s+Likes\.count,\s+Domain\.id,\s+Domain\.settings_id,\s+Domain\.domain,\s+Domain\.created_time\s+`+
			`FROM Likes\s+JOIN Domain\s+ON Likes\.domain_id = Domain\.id\s+`+
			`WHERE Domain\.domain = \?\s+AND Likes\.uri = \?$`,
	).WithArgs("example.com", "/blog/post").
		WillReturnRows(mockRows)
	mock.ExpectQuery(
		`^SELECT \* FROM Liked_IPs\s+WHERE Liked_IPs\.ip = \?\s+AND Liked_IPs\.domain = \?\s+AND Liked_IPs\.path = \?$`,
	).WithArgs("192.0.2.1:1234", "example.com", "/blog/post").WillReturnError(sql.ErrNoRows)
	...
	mock.ExpectExec(
		`^INSERT INTO Liked_IPs\(domain, path, ip, count, created_time\)\s+`+
			...
			`DO UPDATE\s+`+
			`SET count = count \+ 1$`,
	).WithArgs("example.com", "/blog/post", "192.0.2.1:1234").
...
func TestGetClapsWithBodyForAlreadyLiked(t *testing.T) {
	...
	likedIPscolumns := []string{
		...
		"path",       // Liked_IPs.path
		"count",      // Liked_IPs.count (example additional column)
	...
	likedipMockRows := sqlmock.NewRows(likedIPscolumns).
		AddRow(
			...
			"example.com",
			"/blog/post",
			time.Date(2023, time.January, 1, 0, 0, 0, 0, time.UTC), // created_at (use time.Time, NOT string!)
		)
	mock.ExpectQuery(
		`^SELECT \* FROM Liked_IPs\s+WHERE Liked_IPs\.ip = \?\s+AND Liked_IPs\.domain = \?\s+AND Liked_IPs\.path = \?$`,
